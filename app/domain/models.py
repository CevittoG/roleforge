...
class ApplicationStatus(str, Enum):
    """Application lifecycle. Stored as plain string in the Sheet (column D);
    enum is only enforced at the API boundary so old rows with unrecognized
    values keep round-tripping.

    GENERATED..ON_HOLD are user-editable. ERROR is system-set: written when a
    generate run fails (the row + Drive folder are still persisted so the run
    can be re-generated), and flipped to GENERATED by a successful regen. It is
    intentionally kept out of the manual status dropdown on the frontend."""
...
# The raw questions text, saved verbatim so a failed run can be re-generated
# without re-pasting. Internal regen input — not exposed as a download.
APPLICATION_QUESTIONS_INPUT = "Application_Questions_Input.txt"
