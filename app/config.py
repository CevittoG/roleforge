...
class Settings(BaseSettings):
    ...
    # --- Google Gemini (optional second LLM provider; free tier) ---
    # Distinct from the GOOGLE_* OAuth/Drive creds below — this is the Gemini
    # API key. Leave blank to disable the Gemini provider entirely (the UI then
    # hides the selector and falls back to Anthropic).
    gemini_api_key: str = ""
    gemini_model: str = "gemini-3.5-flash"
    gemini_max_tokens: int = 12_000
    # Which provider the UI pre-selects ("anthropic" | "gemini"). If the chosen
    # provider has no API key configured, generation and the UI fall back to an
    # available provider.
    default_llm_provider: str = "gemini"

    # --- Resume header (contact identity) ---
    # The resume header is injected deterministically from here, NOT generated by
    # the model — contact data must never be hallucinated or go stale. Leave blank
    # to omit a field. `resume_links` is comma-separated (LinkedIn, portfolio, …).
