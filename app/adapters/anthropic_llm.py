...
parse; layout is never taken from the model (the docx/Markdown renderers own
layout). The audit block also carries the match/gap analysis used for
cross-opportunity comparison and "what to learn next".

Two entry points share the same cached prefix (persona + anti-hallucination +
experience docs):
...
class AnthropicAdapter:
    ...
    def generate(
        ...
        raw = self._call(SKILL_SYSTEM_PROMPT, experience_docs, jd, jd_suffix=suffix)
        return self._parse(raw)

    def generate_interview_prep(self, *, experience_docs: str, jd: JobDescription) -> str:
        raw = self._call(INTERVIEW_PREP_SYSTEM_PROMPT, experience_docs, jd)
        return _strip_fences(raw).strip()

    # --- internals ---
    def _call(
        self, system_prompt: str, experience_docs: str, jd: JobDescription, *, jd_suffix: str = ""
    ) -> str:
